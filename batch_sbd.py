...
if __name__ == '__main__':
    ...
    usage = 'usage: %prog [options] <input_file_list> <output_file_list>'
    ...
    ## get test file
    if len(args) == 2:
        options.test, options.output = args
        if not os.path.isfile(options.test): sbd_util.die('test path [%s] does not exist' %options.test)
    else:
        sbd_util.die('you have to specifiy an input file list and an output file list')

    input_files  = list(open(options.test).readlines())
    output_files = list(open(options.output).readlines())

    if not len(input_files) == len(output_files):
        sbd_util.die('you should have the same number of input and output files')
    ...
    for i in range(len(input_files)):
        #try:
        test = get_data(input_files[i].strip(), tokenize=True)
        out_dir = os.path.dirname(output_files[i].strip())
        try:
            os.makedirs(out_dir)
        except:
            pass
        out  = open(output_files[i].strip(), 'w')
        test.featurize(model, verbose=True)
        model.classify(test, verbose=True)
        test.segment(use_preds=True, tokenize=options.tokenize, output=out)
        #except:
        #    print('Error processing file:' + input_files[i].strip())
